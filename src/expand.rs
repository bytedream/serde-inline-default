...
const SERDE_INLINE_PARENT: &str = "serde_inline_default_parent";
    ...
    for (i, field) in item.fields.iter_mut().enumerate() {
        for (j, attr) in field.attrs.iter_mut().enumerate() {
            if attr.path().is_ident(SERDE_INLINE_DEFAULT) {
                ...
                let parent_default_field_name = field.clone().ident.expect("missing parent field");
                let default: TokenStream =
                    format!("{}::default().{}", item.ident, parent_default_field_name)
                        .parse()
